...
```
This is a function which takes a sequence from the COVID data I compiled from NCBI as well as a kmer length `k` and returns which kmers of that given length exist.
...
It computes quite quickly, an important point of comparison for two sequences.
...
Although this kmer value is fairly low, it allows for this analyses to show differences in unique kmers across time periods, which is why I chose to use it.

### Using my data for this function
When I used my data for this function, I got a very large set of values since it is evaluating a small `k` value for a large number of sequences.
This however is good, because there are many points of comparison.
...
When you really want to see changes with big data, you might as well use a plot that can break down the large data into clear points.
...
It is also a perfect visualization for an x-variable that is categorical and a y-value that is numerical.
...
The graph basically showed that as time went on, the number of unique kmer counts increased.
...
To me, this indicates that the sequences in my data changed very much during the last time period in order to incur such a variation in such a small `k` value.
...
When  I began, I had a lot of trouble with this function.
That's because I was attempting to create a matrix from values I had retrieved from previous functions.
...
However, that turned out to be complicated and time consuming.
Therefore, I tried to create a skeleton function imitating out previous Needleman Wunch assignment as a reference.
...
Here, and with some office hours guidance, I discovered the use of `i <= j && continue` this indicates that the function should retrieve the values only if `i` is less than or equal to `j` in order to only retrieve data from the lower triangle and skipping the upper triangle since the values would be equivalent.
...
It took a bit of thinking through to realize that I would indice through 36 as only half of the matrix is being used for pairing calculations.
